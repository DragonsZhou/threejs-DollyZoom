...
function render() {
  ...
  if (curPos >= 0) {
    curPos -= 0.001;
    var tmp = DollyZoom(new THREE.Vector3(0, 5, 5), new THREE.Vector3(0, 5, 12), new THREE.Vector3(0, 0, 0), 120, curPos); // oneDo(()=>{console.log(tmp);})
...
function DollyZoom(nearPoint, farPoint, targetPos, nearFov, curProgress) {
  var line1 = new THREE.Line3(nearPoint, farPoint);
  var targetPosProgress = line1.closestPointToPointParameter(targetPos); // console.log(targetPosProgress);

  var targetPoint = line1.at(targetPosProgress, new THREE.Vector3()); // console.log(targetPoint);

  var line2 = new THREE.Line3(nearPoint, targetPoint);
  var distance2 = line2.distance(); // console.log(distance2);

  var currentPoint = line1.at(curProgress, new THREE.Vector3()); // console.log(currentPoint);

  var line3 = new THREE.Line3(currentPoint, targetPoint);
  var distance3 = line3.distance(); // console.log(distance3);

  var radFov = nearFov / 2 * Math.PI / 180;
  var curFov = Math.atan(Math.tan(radFov) * distance2 / distance3) / Math.PI * 180 * 2;
  return {
...
function init(el, width, height) {
  ...
  camera.position.set(0, 7, 20); // camera.lookAt(0, 15, 0)

  camera.updateProjectionMatrix(); // const controls = new OrbitControls(camera, renderer.domElement);
